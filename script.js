...
const game = {
    ...
    start() {
        ...
        board.render();
        snake.parts = snake.startParts.slice();
        snake.render();
        food.items = food.startItems.slice();
        food.render();
    },
    ...
    /**
     * Количество нажатий на кнопку "Пауза".
     */
    pauseCount: 0,
    ...
    stop() {
        ...
        alert("Игра остановлена, Вы проиграли");
        game.start();
    ...
    move(event) {
        ...
        /* проверяем врезается ли змейка сама в себя, и если так то останавливаем игру */
        for (let item of snake.parts) {
            if (item.top == nextPosition.top && item.left == nextPosition.left) {
                game.stop();
                return;
            }
        }
